...
private extension Logger {
    static let nfc = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.frick", category: "NFCReader")
}
...
class NFCReader: NSObject, ObservableObject, NFCNDEFReaderSessionDelegate {
    ...
    private func startSession() {
        guard NFCNDEFReaderSession.readingAvailable else {
            Logger.nfc.warning("NFC is not available on this device")
            return
        }
    ...
    func readerSession(_ session: NFCNDEFReaderSession, didInvalidateWithError error: Error) {
        if let readerError = error as? NFCReaderError {
            if (readerError.code != .readerSessionInvalidationErrorFirstNDEFTagRead)
                ...
                Logger.nfc.error("Session invalidated with error: \(error.localizedDescription)")
            }
        }
        self.session = nil
    }
