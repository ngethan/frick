...
private extension Logger {
    static let profileManager = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.frick", category: "ProfileManager")
}
...
class ProfileManager: ObservableObject {
    ...
    var currentProfile: Profile {
        profiles.first(where: { $0.id == currentProfileId }) ?? profiles.first(where: { $0.name == "Default" }) ?? Profile(name: "Default", appTokens: [], categoryTokens: [], icon: "🔕")
    }
    ...
    func loadProfiles() {
        if let savedProfiles = UserDefaults.standard.data(forKey: "savedProfiles"),
           let decodedProfiles = try? JSONDecoder().decode([Profile].self, from: savedProfiles) {
            ...
            let defaultProfile = Profile(name: "Default", appTokens: [], categoryTokens: [], icon: "🔕")
        ...
        if let savedProfileId = UserDefaults.standard.string(forKey: "currentProfileId"),
           let uuid = UUID(uuidString: savedProfileId) {
            ...
            Logger.profileManager.info("Found currentProfile: \(uuid)")
        } else {
            currentProfileId = profiles.first?.id
            Logger.profileManager.info("No stored ID, using \(self.currentProfileId?.uuidString ?? "NONE")")
        }
    }
    ...
    func addProfile(name: String, icon: String = "🔕") {
    ...
    func setCurrentProfile(id: UUID) {
        if profiles.contains(where: { $0.id == id }) {
            currentProfileId = id
            Logger.profileManager.info("New Current Profile: \(id)")
            saveProfiles()
        }
    }
...
struct Profile: Identifiable, Codable {
    ...
    // Updated initializer to support emoji
    init(name: String, appTokens: Set<ApplicationToken>, categoryTokens: Set<ActivityCategoryToken>, icon: String = "🔕") {
        self.id = UUID()
        self.name = name
